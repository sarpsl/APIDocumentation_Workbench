...
# --- Unsloth integration (must be first) ---
use_unsloth = False
try:
    from unsloth import patch_hf_for_unsloth, FastTrainer
    patch_hf_for_unsloth()
    use_unsloth = True
except (ImportError, NotImplementedError) as e:
    print(f"[INFO] Unsloth not available or not supported: {e}\nFalling back to standard Hugging Face Trainer.")
...
def objective(trial):
    ...
    trainer_cls = FastTrainer if use_unsloth else Trainer
    trainer = trainer_cls(
        model=model,
        args=training_args,
